...
> 当前实现口径：MVP1.0 通用投标闭环已完成；MVP1.1 P0 + P1 已完成模型配置、PromptOps、合规矩阵 AI 增强、原文审阅、人工补漏、相似补票和重复项级联确认。多 Agent 编排、企业资料真实 RAG、技术标核心章节、产品选型和图纸仍顺延。
...
长期目标仍可采用“业务 Agent + Harness 编排 + 工具能力 + 数据治理 + 人工审批”的分层架构。当前 MVP1.1 实现先采用“受控模型任务 + 服务层编排 + 人工关口”的方式落地，不把多 Agent 编排作为当前验收范围。
