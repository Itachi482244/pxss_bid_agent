...
## 7. MVP1.2、MVP1.3 与长期规划顺延清单

以下内容不建议塞回 MVP1.1。MVP1.2 只承接 ContextPack 主链路：
...
MVP1.3 承接 ContextPack 的上游证据增强能力：
...
明珠公寓真实 PDF 已完成完整矩阵生成和质量门禁验收，最终质量报告通过且无剩余质量问题。下一步建议进入 MVP1.2：商务/资格草稿生成 ContextPack、分章节提示词、结构化生成稿和审阅闭环；MVP1.3 再接企业资料 RAG、Embedding/Rerank 和候选证据推荐。技术标、净化产品选型、图纸/OCR、多 Agent 和报价辅助进入长期规划，不再继续往 MVP1.1 塞新功能。
