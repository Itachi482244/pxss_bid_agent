...
from app.models import Document, DocumentChunk, DocumentVersion, EnterpriseMaterial, Project, User
...
def test_tender_format_docx_export_supports_review_and_submission_modes(monkeypatch) -> None:
    ...
    client = TestClient(app)
    project_id, section_id = get_seed_project_and_section(client)
    _ensure_exportable_tender_doc(project_id, section_id)
    ...
    review = client.post(base, json={"export_mode": "review"})
    ...
    submission = client.post(base, json={"export_mode": "submission"})
